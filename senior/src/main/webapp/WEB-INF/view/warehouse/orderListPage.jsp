...
<script type="text/javascript">
    layui.use(['table','layer','form'],function () {
        ...
        table.render({
            id:"tableReload",
            elem:'#productList',
            url:'/senior/warehouse/getOrderList',
            even:true,
            page:true,
            limit: 10,
            limits: [5,10,20,30],
        ...
        table.on('tool(productListFilter)', function(obj){
            ...
            if(obj.event === 'orderDetail'){
                layer.open({
                    ...
                    btn:["关闭"],
                    btnAlign: 'c',
                    content: '/senior/warehouse/getOrderDetailByOrderId?orderId=' + data.orderId,
                ...
                layer.confirm('确认发货吗', function(index){
                    $.ajax({
                        url:"/senior/warehouse/deliveryOrder",
                        type:"post",
                        data:{orderId:data.orderId},
                        dataType:"json",
                        success:function (result) {
                            if(result.success){
                                layer.msg("发货成功！",{icon: 6,time:1000,end:function(){
                                    table.reload('tableReload',{page:{curr:1}});
                                    layer.close(index);
                                }});
                            }else{
                                layer.msg("发货失败！",{icon: 5,time:1000,end:function(){
                                    table.reload('tableReload',{page:{curr:1}});
                                    layer.close(index);
                                }});
                            }
